            ...
            <Row style={{ paddingLeft: "5px" }}>
                <Form>
                    ...
                    <Row className={(Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? 'col-sm-8 col-md-8' : 'col-sm-8 col-md-8'} style={{ whiteSpace: "nowrap", overflow: "hidden" }}>
                        <Col className={(Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? 'col-sm-3 col-md-4' : 'col-sm-3 col-md-4'} >
                            <Form.Group  >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{(Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? t('customer') : t('customershort')}&nbsp;:</Form.Label>
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.customer : ""}</Form.Label>
                            </Form.Group>
                            <Form.Group >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{t('address')}&nbsp;&nbsp;&nbsp;:</Form.Label>
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.address : ""}</Form.Label>
                            </Form.Group>
                            <Form.Group >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{t('loannoshort')}&nbsp;:</Form.Label>
                                <Form.Label style={{ "fontWeight": "bold", marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.loannumber : ""}</Form.Label>
                            </Form.Group>
                        </Col>
                        <Col className={(Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? 'col-sm-3 col-md-4' : 'col-sm-3 col-md-4'} style={{ paddingLeft: "2px" }} >
                            <Form.Group  >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{relationtype == 0 ? (Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? t('fathername') : t('fathershort') : (Number(process.env.REACT_APP_LOAN_APP_STARTDATE) === 0) ? t('husbandname') : t('husbandshort')}&nbsp;:</Form.Label>
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.fathername : ""}</Form.Label>
                            </Form.Group>
                            <Form.Group  >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{t('city')}&nbsp;&nbsp;:</Form.Label>
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.referencecity : ""}</Form.Label>
                            </Form.Group>
                            <Form.Group  >
                                <Form.Label style={{ fontSize: fontsizevar, marginBottom: "0" }}>{t('total')}&nbsp;&nbsp;:</Form.Label>
                                <Form.Label style={{ "fontWeight": "bold", marginBottom: "0" }}>&nbsp;{ledger.length > 0 ? first.totalamount : ""}</Form.Label>
                            </Form.Group>
                        </Col>
                    </Row>
                </Form>
